...
impl<T: Ord> LinkedList<T> {
    ...
    pub fn merge(mut list_a: LinkedList<T>, mut list_b: LinkedList<T>) -> Self {
        ...
        while list_a.start.is_some() && list_b.start.is_some() {
            let aval = &unsafe { &*list_a.start.unwrap().as_ptr() }.val;
            let bval = &unsafe { &*list_b.start.unwrap().as_ptr() }.val;
            if aval < bval {
                let node_ptr = list_a.start;
                list_a.start = unsafe { (*node_ptr.unwrap().as_ptr()).next };

                match merged_list.end {
                    None => merged_list.start = node_ptr,
                    Some(end_ptr) => unsafe { (*end_ptr.as_ptr()).next = node_ptr },
                }
                merged_list.end = node_ptr;
                merged_list.length += 1;
            } else {
                let node_ptr = list_b.start;
